...
`ToolkitIntegrationApiContractTest` is part of `ProductionTestRunner`.
It protects the versioned JSON boundary used by the JavaFX application:

- API version handshake;
- content filtering (`trade-evolution` stays outside HOTKEY composition);
- predictable empty-selection and missing-seed diagnostics;
- the known LG1.0 BOX14 + Repel + Party IV + Run+Bike plan/capacity;
- real WC3 artifact emission and preservation of the base input.

This is a source/build contract test. It does not upgrade any gameplay path to
GAME-VALIDATED.


## Desktop integration contract additions

`ToolkitIntegrationApiContractTest` also freezes:

- preset-owned parameter metadata (`seed: HEX_U32`) and generic `--param`;
- legacy `--seed` compatibility;
- invalid/missing parameter diagnostics;
- BOX14 Seed requiring no build parameter;
- generic `RESOURCE_CONFLICT` for the shared mobility EWRAM owner;
- `HOTKEY_CONFLICT` for duplicate default bindings;
- resource allocations returned in a successful plan.


## Cross-version BOX14 / mobility port gate

`CrossVersionMobilityAndBox14PortTest` is part of `ProductionTestRunner`. For `fr10`, `lg10`, `fr11`, and `lg11` it verifies that `seed-modifier-box14`, `run-anywhere`, and `run-bike-anywhere` are catalog-supported; standalone plans materialize; representative Shared plans materialize; the mobility presets still collide through the generic `mobility-ewram-sidecar` resource; and non-LG1.0 ports remain `SUPPORTED_NOT_TESTED`. The full historical runner now reports 372 tests plus `TradeEvolutionPresetProductionTest`.

The port evidence uses the supplied pret symbol maps. All four map `gWirelessCommType` to `03003F3C`, `gPokemonStoragePtr` to `03005010`, `sFlickerArray` to `02022AE8` (size `0x64`), `gMapHeader` to `02036DFC`, and `gMain` to `030030F0`. This is build/source evidence only.
