...
/*
   ...
   Probe 3F deliberately uses only one enemy mon. Six custom enemy slots were
...
*/
final class BrockGymLeaderBgmProbeNative {
    ...
    private BrockGymLeaderBgmProbeNative() {}
    ...
    static NativeHelper buildAt(RomProfile rom, long stagingAddress) {
        ...
        // Probe 3H: force the FR/LG Gym Leader battle theme explicitly.
        // The e-Reader bootstrap still has opponent id 0 at this point, so
        // song=0 would ask GetBattleBGM() too early and select VS Trainer.
        t.ldrLit(0, "gymLeaderBattleBgm"); // MUS_VS_GYM_LEADER = 0x0128
        ...
        t.adr(1, "defeat_text");
        t.u16(0x6001);                    // sTrainerADefeatSpeech = staged custom text
        t.u16(0xBD00);                    // pop {pc}
        ...
        t.literal("gymLeaderBattleBgm", 0x0128L);
        ...
        t.label("defeat_text");
        byte[] defeatText = Gen3TextCodec.encodeString("Not bad!");
        t.raw(defeatText);
        if ((t.pos() & 1) != 0)
            t.raw(new byte[] {0});
