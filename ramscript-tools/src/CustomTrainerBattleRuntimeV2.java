...
*/
final class CustomTrainerBattleRuntimeV2 {
    ...
    private CustomTrainerBattleRuntimeV2() {}

    static long stagingAddress(RomProfile rom) { return rom.stringVar4 + 0x140L; }

    static NativeHelper build(RomProfile rom) { return buildLayout(rom).helper(); }

    private static Layout buildLayout(RomProfile rom) {
        ...
        t.ldrLit(0, "trainerDefeatSpeech"); t.ldrLit(1, "defeatBuffer"); t.u16(0x6001);
        t.u16(0xBD00);
        ...
        // Shared service used by every descriptor card. VAR_8004 selects which
        // SB1 overworld string to copy into gStringVar4 for stock `message`.
        t.align4();
        int fieldTextReaderOffset = t.pos();
        t.label("field_text_reader");
        t.u16(0xB570); // push {r4-r6,lr}
        t.ldrLit(4, "saveBlock1Ptr"); t.u16(ldrWord(4,4,0));
        t.ldrLit(1, "fieldTextOffset"); t.u16(addReg(4,4,1));
        t.ldrLit(0, "specialVar8004"); t.u16(ldrHalf(0,0,0));
        t.u16(0x2800); t.bcond(0, "field_text_pre");
        t.u16(0x2801); t.bcond(0, "field_text_post");
        t.u16(ldrHalf(1,4,CustomTrainerFieldTextStorage.OFF_ALREADY_COMPLETED));
        t.b("field_text_selected");
        t.label("field_text_pre");
        t.u16(ldrHalf(1,4,CustomTrainerFieldTextStorage.OFF_PRE_BATTLE));
        t.b("field_text_selected");
        t.label("field_text_post");
        t.u16(ldrHalf(1,4,CustomTrainerFieldTextStorage.OFF_POST_VICTORY));
        t.label("field_text_selected");
        t.u16(addReg(4,4,1));
        t.ldrLit(5, "stringVar4");
        t.label("field_text_copy");
        t.u16(ldrByte(0,4,0)); t.u16(strByte(0,5,0));
        t.u16(0x28FF); t.bcond(0, "field_text_done");
        t.u16(0x3401); t.u16(0x3501); t.b("field_text_copy");
        t.label("field_text_done"); t.u16(0xBD70);
        t.align4();

        t.literal("saveBlock2Ptr", rom.saveBlock2Ptr);
        t.literal("saveBlock1Ptr", rom.saveBlock1Ptr);
        t.literal("fieldTextOffset", CustomTrainerFieldTextStorage.OFFSET);
        t.literal("specialVar8004", rom.specialVar8004);
        t.literal("stringVar4", rom.stringVar4);
        t.literal("defeatBuffer", defeatBufferAddress(rom));
        ...
        if (code.length > CustomTrainerRuntimeStorage.RUNTIME_CAPACITY)
            throw new IllegalStateException("CustomTrainerBattleRuntimeV2 too large: " + code.length + "/" + CustomTrainerRuntimeStorage.RUNTIME_CAPACITY);
        return new Layout(new NativeHelper(stagingAddress, code), fieldTextReaderOffset);
    }

    private record Layout(NativeHelper helper, int fieldTextReaderOffset) {}

    static long fieldTextReaderAddress(RomProfile rom) {
        Layout layout = buildLayout(rom);
        return layout.helper().stagingAddress() + layout.fieldTextReaderOffset() + 1L;
    }

    private static long defeatBufferAddress(RomProfile rom) { return rom.stringVar4 + 0x370L; }
    ...
    private static final class Thumb {
        ...
        void bcond(int cond,String label){int p=pos();u16(0xD000|(cond<<8));branches.add(new BranchFix(p,label));} void b(String label){int p=pos();u16(0xE000);branches.add(new BranchFix(p,label));}
        ...
        byte[] finish(){align4();Map<String,Integer> lp=new HashMap<>();for(var e:literals.entrySet()){lp.put(e.getKey(),pos());long v=e.getValue();out.write((int)v&255);out.write((int)(v>>>8)&255);out.write((int)(v>>>16)&255);out.write((int)(v>>>24)&255);}byte[]b=out.toByteArray();for(var f:branches){int target=req(f.label),d=target-(f.pos+4),hw=d/2;if((d&1)!=0)throw new IllegalStateException("branch alignment "+f.label+" d="+d);int ins=(b[f.pos]&255)|((b[f.pos+1]&255)<<8);if((ins&0xF800)==0xE000){if(hw< -1024||hw>1023)throw new IllegalStateException("branch range "+f.label+" d="+d);put16(b,f.pos,0xE000|(hw&0x7FF));}else{if(hw< -128||hw>127)throw new IllegalStateException("branch range "+f.label+" d="+d);put16(b,f.pos,(ins&0xFF00)|(hw&255));}}for(var f:bls){int target=req(f.label),d=target-(f.pos+4);if((d&1)!=0||d<-(1<<22)||d>=(1<<22))throw new IllegalStateException("BL range "+f.label);put16(b,f.pos,0xF000|((d>>12)&0x7FF));put16(b,f.pos+2,0xF800|((d>>1)&0x7FF));}for(var f:literalLoads){Integer p=lp.get(f.name);if(p==null)throw new IllegalStateException("missing literal "+f.name);int pc=(f.pos+4)&~3,d=p-pc;if(d<0||(d&3)!=0||d/4>255)throw new IllegalStateException("literal range "+f.name+" d="+d);put16(b,f.pos,0x4800|(f.rt<<8)|(d/4));}for(var f:adrs){int target=req(f.label),pc=(f.pos+4)&~3,d=target-pc;if(d<0||(d&3)!=0||d/4>255)throw new IllegalStateException("ADR range "+f.label+" d="+d);put16(b,f.pos,0xA000|(f.rd<<8)|(d/4));}return b;}
