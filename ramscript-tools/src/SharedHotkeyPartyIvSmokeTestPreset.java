...
final class SharedHotkeyPartyIvSmokeTestPreset {
    ...
    static Layout layout(RomProfile rom, int seed) {
        ...
        PersistentNativeCallBridge.Build partyBridgeBuild = buildPartyBridge(rom);
        ...
        // The Party IV bridge embeds a NativeHelperInstaller CPU_SET_BLOCK raw
        // source. cpuCandidate() aligns the raw block relative to the script,
        // so the script base itself must also be word-aligned. The previous
        // Build 23 placement started this bridge at SB2+0x0D5F (odd), making
        // the physical CpuSet source misaligned even though its relative
        // offset was aligned.
        cursor = align(cursor, partyBridgeBuild.requiredBaseAlignment());
    ...
    private static PersistentNativeCallBridge.Build buildPartyBridge(RomProfile rom) {
