...
final class MewBattleCoreNative {
    ...
    private MewBattleCoreNative(){}

    record Layout(NativeHelper helper,int restoreOffset,int launchOffset){}
    ...
    static Layout buildAt(RomProfile rom,long addr){
        ...
        t.u16(0xB5F0);
        // GAME-VALIDATED Probe-4 move override: Psychic / Recover / Thunderbolt / Ice Beam.
        for (int slot = 0; slot < MOVES.length; slot++) {
            t.ldr(0,"enemyParty");
            t.mov(1,MOVES[slot]);
            t.mov(2,slot);
            t.ldr(3,"setMonMoveSlot");
            t.bl("thunk_r3");
        }
        ...
        t.lit("enemyParty",G_ENEMY_PARTY);t.lit("setMonMoveSlot",0x0803E965L);
        return new Layout(new NativeHelper(addr,t.finish()),restore,launch);
    }
