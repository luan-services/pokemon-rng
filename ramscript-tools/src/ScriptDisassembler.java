    ...
    private Decoded decodeOperands(int pc, int opcode, ScriptOpcode.OperandFormat format) {
        ...
        switch (opcode) {
            ...
            case 0x18: { // subvar
                int variable = Binary.u16(script, p);
                int value = Binary.u16(script, p + 2);
                return new Decoded(Vars.format(variable) + ", " + formatVarOrValue(value), "");
            }
            case 0x19: { // copyvar
                int destination = Binary.u16(script, p);
                int source = Binary.u16(script, p + 2);
                return new Decoded(Vars.format(destination) + ", " + Vars.format(source), "");
            }
            case 0x22: { // compare_var_to_var
                int first = Binary.u16(script, p);
                int second = Binary.u16(script, p + 2);
                return new Decoded(Vars.format(first) + ", " + Vars.format(second), "");
            }
            case 0x25: { // special
            ...
            case 0x23: // callnative
            case 0x24: { // gotonative
                long ptr = Binary.u32(script, p);
                return new Decoded(hex32(ptr), "absolute native function address; ROM-build dependent");
            }
            case 0x79: { // givemon
                int species = Binary.u16(script, p);
                int level = Binary.u8(script, p + 2);
                int item = Binary.u16(script, p + 3);
                long param1 = Binary.u32(script, p + 5);
                long param2 = Binary.u32(script, p + 9);
                int param3 = Binary.u8(script, p + 13);
                return new Decoded(
                        hex16(species) + ", " + level + ", " + Items.format(item)
                                + ", " + hex32(param1) + ", " + hex32(param2) + ", " + hex8(param3),
                        "givemon raw parameters"
                );
            }
            case 0x7A: { // giveegg
                int species = Binary.u16(script, p);
                return new Decoded(hex16(species), "");
            }
            case 0x90: // addmoney
            case 0x91: // removemoney
            case 0x92: { // checkmoney
                long amount = Binary.u32(script, p);
                int ignore = Binary.u8(script, p + 4);
                return new Decoded(Long.toUnsignedString(amount) + ", " + ignore, "");
            }
            case 0xB6: { // setwildbattle
                int species = Binary.u16(script, p);
                int level = Binary.u8(script, p + 2);
                int item = Binary.u16(script, p + 3);
                return new Decoded(hex16(species) + ", " + level + ", " + Items.format(item), "");
            }
            case 0xBD: { // vmessage
                long ptr = Binary.u32(script, p);
        ...
        return switch (format) {
            ...
            case U32_U8 -> new Decoded(
                    hex32(Binary.u32(script, p)) + ", " + hex8(Binary.u8(script, p + 4)), ""
            );
            case U16_U8_U16 -> new Decoded(
                    hex16(Binary.u16(script, p)) + ", " + hex8(Binary.u8(script, p + 2))
                            + ", " + hex16(Binary.u16(script, p + 3)), ""
            );
            case U16_U8_U16_U32_U32_U8 -> new Decoded(
                    hex16(Binary.u16(script, p)) + ", " + hex8(Binary.u8(script, p + 2))
                            + ", " + hex16(Binary.u16(script, p + 3))
                            + ", " + hex32(Binary.u32(script, p + 5))
                            + ", " + hex32(Binary.u32(script, p + 9))
                            + ", " + hex8(Binary.u8(script, p + 13)), ""
            );
