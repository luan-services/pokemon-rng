/*
   Persistent Event Framework — Probe 5.
...
*/
final class PersistentEventMewRecoveryProbePreset {
    ...
    static final ObjectEventTarget JULIA = new ObjectEventTarget(
            "route8-julia-probe5", "Route 8 — Julia (Probe 5 host)",
            3, 26, 1, 62, 14,
    ...
    static final ObjectEventTarget MEW_HOST = new ObjectEventTarget(
            "route8-cut-tree-b-mew-host-probe5", "Route 8 — Cut Tree B / Mew host",
            3, 26, 11, 47, 12,
            "EventScript_CutTree", ObjectEventSafety.SAFE_SIMPLE_DIALOG);

    private PersistentEventMewRecoveryProbePreset() {}

    static TriggerBuildResult build(RomProfile rom) {
        if (rom != RomProfile.FIRE_RED_EN_10)
            throw new IllegalArgumentException("Persistent Event Probe 5 is intentionally FR1.0-only");

        byte[] eventPackage = buildPackage(rom);
        ...
        NativeHelperInstaller.Plan packagePlan = NativeHelperInstaller.prepare(
                ...
                "persistent_event_probe5_package", NativeHelperInstaller.Mode.AUTO);
        NativeHelperInstaller.Plan transformPlan = NativeHelperInstaller.prepare(
                b, VIRTUAL_BASE, transformAndRebind, copier,
                "persistent_event_probe5_transform_rebind", NativeHelperInstaller.Mode.AUTO);
    ...
    static byte[] buildPackage(RomProfile rom) {
        long copier = rom.stringVar4 + 0x100L;
        long helperAddr = CpuSetNativeHelperInstaller.helperDestination(copier);
        NativeHelper ensureMew = buildEnsureMewHelper(rom, helperAddr);

        RamScriptBuilder p = new RamScriptBuilder(PACKAGE_VIRTUAL_BASE);
        p.setVAddress();
        NativeHelperInstaller.Plan ensurePlan = NativeHelperInstaller.prepare(
                p, PACKAGE_VIRTUAL_BASE, ensureMew, copier,
                "persistent_event_probe5_ensure_mew", NativeHelperInstaller.Mode.AUTO);

        p.lockAll()
         // Works for both paths: visible Mew, or vanilla Cut Tree restored after map reload.
         .applyMovement(MEW_HOST.localId(), commonMovementExclamationMark(rom))
         .waitMovement(MEW_HOST.localId());
        ensurePlan.installAndCall(p);
        p.vMessage("mewMsg").waitMessage().waitButtonPress()
         .releaseAll().end()
         .text("mewMsg", "Miuu!");
    ...
    private static NativeHelper buildEnsureMewHelper(RomProfile rom, long address) {
        ...
        final int literalOffset = 0x30;
        byte[] c = new byte[0x48];
        putU16(c,0x00,0xB5F0); // push {r4-r7,lr}
        putU16(c,0x02,ldrLiteral(4,0x02,literalOffset));
        putU16(c,0x04,0x6824); // live SB1
        putU16(c,0x06,ldrLiteral(5,0x06,literalOffset+4));
        putU16(c,0x08,0x1965); // templates
        putU16(c,0x0A,0x263F);
        putU16(c,0x0C,0x7828);
        putU16(c,0x0E,0x2800 | MEW_HOST.localId());
        putU16(c,0x10,branchCond(0,0x10,0x1C));
        putU16(c,0x12,0x3518);
        putU16(c,0x14,0x3E01);
        putU16(c,0x16,branchCond(2,0x16,0x0C));
        putU16(c,0x18,branchUncond(0x18,0x20));
        putU16(c,0x1A,0x46C0);
        putU16(c,0x1C,0x208C); // OBJ_EVENT_GFX_MEW
        putU16(c,0x1E,0x7068); // template.graphicsId = Mew
        // Always update the live object too.
        ...
        putU16(c,0x2A,0xF000); putU16(c,0x2C,0xF807); // BL thunk @ 0x3C
        putU16(c,0x2E,0xBDF0);
    ...
    static NativeHelper buildPackageInstaller(RomProfile rom, long address, byte[] data) {
        ...
        putU16(code,0x0A,ldrLiteral(2,0x0A,literalOffset+12));
        ...
        putU32(code,literalOffset+8,address+dataOffset); putU32(code,literalOffset+12,data.length);
