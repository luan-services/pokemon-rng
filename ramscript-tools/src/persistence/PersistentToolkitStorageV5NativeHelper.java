...
*/
final class PersistentToolkitStorageV5NativeHelper {
    private PersistentToolkitStorageV5NativeHelper() {}

    static NativeHelper buildInstallerAt(RomProfile rom, long address) {
        byte[] catalog = PersistentToolkitStorageV5.buildCatalogImage(rom);
        byte[] sid = PersistentToolkitStorageV5.buildSaveBlock1Payload(rom);
        if (catalog.length > 0xFF || sid.length > 0xFF) {
            throw new IllegalStateException("Build 9 compact installer length overflow");
        }
        ...
        PersistentToolkitStorageV2.putU32(code,literalOffset + 0x10,PayloadStorageArea.SAVE_BLOCK1.offset() + PersistentToolkitStorageV5.SID_SB1_OFFSET);
    ...
    static NativeHelper buildDispatcherAt(RomProfile rom, long address) {
        ...
        b.emit(0x2800 | PersistentToolkitStorageV5.VERSION);
        b.bCond(1,"fail");
        ...
        b.literal("magic",PersistentToolkitStorageV5.MAGIC);
