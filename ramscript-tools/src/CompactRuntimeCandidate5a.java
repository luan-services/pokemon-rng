...
*/
final class CompactRuntimeCandidate5a {
    ...
    private CompactRuntimeCandidate5a() {}
    ...
    static RamScript build(RomProfile rom, byte[] payload) {
        if (payload == null || payload.length == 0) {
            throw new IllegalArgumentException("compact C5a payload must not be empty");
        }
        ...
        if (p != SIGNATURE_OFFSET) {
            throw new IllegalStateException("compact C5a signature offset mismatch");
        }
        ...
        if (p != PAYLOAD_OFFSET) {
            throw new IllegalStateException("compact C5a payload offset mismatch");
        }
        ...
        if (p != nativeBlobOffset || (nativeBlobOffset & 3) != 0) {
            throw new IllegalStateException("compact C5a native blob alignment mismatch");
        }
        ...
        if (p != fieldInstallerOffset) {
            throw new IllegalStateException("compact C5a field installer offset mismatch");
        }
    ...
    static byte[] nativeInstallerBlob(RomProfile rom) {
        ...
        if (blocks.size() != 12) {
            throw new IllegalStateException("compact C5a expects 12 resident blocks");
        }
        if (RuntimeV1ResidentBlocks.totalResidentBytes(rom) != RESIDENT_DATA_SIZE) {
            throw new IllegalStateException("compact C5a resident byte count mismatch");
        }
        ...
        if (codeAndLiterals.length != NATIVE_CODE_AND_LITERALS_SIZE) {
            throw new IllegalStateException("compact C5a native code size mismatch");
        }
        ...
        for (RuntimeV1ResidentBlocks.Block block : blocks) {
            ...
            if ((address & 0xFFFF0000L) != 0x03000000L) {
                throw new IllegalStateException("compact C5a block outside IWRAM");
            }
        ...
        if (blob.length != NATIVE_BLOB_SIZE) {
            throw new IllegalStateException(
                    "compact C5a expected native blob " + NATIVE_BLOB_SIZE
                            + ", got " + blob.length
            );
        }
        return blob;
