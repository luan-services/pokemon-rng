...
final class CompactInstallerCandidate2a {
    static final long BOOTSTRAP_ADDRESS = 0x03005310L;
    static final int HEADER_SIZE = 0x0A;
    static final int ALIGNMENT_PADDING = 2;
    static final int NATIVE_BLOB_OFFSET = HEADER_SIZE + ALIGNMENT_PADDING; // 0x0C
    ...
    private CompactInstallerCandidate2a() {}
    ...
    static RamScript build(RomProfile rom) {
        ...
        int nativeBlobOffset = NATIVE_BLOB_OFFSET;
        ...
        if (p != HEADER_SIZE) {
            throw new IllegalStateException("compact C2a header size mismatch");
        }

        // Two data bytes skipped by vgoto. They align the native installer
        // blob to a 4-byte boundary in the physical saved RamScript.
        script[p++] = 0;
        script[p++] = 0;

        if (p != nativeBlobOffset) {
            throw new IllegalStateException("compact C2a native blob alignment mismatch");
        }
    ...
    static int scriptSize(RomProfile rom) {
        return NATIVE_BLOB_OFFSET + NATIVE_BLOB_SIZE + fieldInstallerSize(rom);
    }
