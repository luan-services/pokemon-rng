/*
   Persistent Event Framework — Probe 3.
   ...
   to OBJ_EVENT_GFX_MEW and also updates the live object if it is loaded. The
...
*/
final class PersistentEventMewHostProbePreset {
    ...
    static final ObjectEventTarget JULIA = new ObjectEventTarget(
            "route8-julia-probe3", "Route 8 — Julia (Probe 3 host)",
            3, 26, 1, 62, 14,
    ...
    static final ObjectEventTarget MEW_HOST = new ObjectEventTarget(
            "route8-cut-tree-b-mew-host-probe3", "Route 8 — Cut Tree B / Mew host",
            3, 26, 11, 47, 12,
            "EventScript_CutTree", ObjectEventSafety.SAFE_SIMPLE_DIALOG);

    private PersistentEventMewHostProbePreset() {}

    static TriggerBuildResult build(RomProfile rom) {
        if (rom != RomProfile.FIRE_RED_EN_10)
            throw new IllegalArgumentException("Persistent Event Probe 3 is intentionally FR1.0-only");
        ...
        NativeHelperInstaller.Plan packagePlan = NativeHelperInstaller.prepare(
                ...
                "persistent_event_probe3_package", NativeHelperInstaller.Mode.AUTO);
        NativeHelperInstaller.Plan transformPlan = NativeHelperInstaller.prepare(
                b, VIRTUAL_BASE, transformAndRebind, copier,
                "persistent_event_probe3_transform_rebind", NativeHelperInstaller.Mode.AUTO);
        ...
        b.vMessage("juliaMsg").waitMessage().waitButtonPress()
