/*
   Native RamScript Runtime v1 RC4 (FireRed English 1.0)
...
*/
final class NativeRamScriptRuntimeV1Rc4 {
    ...
    // gHostRfuGameData. RC4 uses 10 bytes for the hotkey safety gate.
    ...
    private NativeRamScriptRuntimeV1Rc4() {}
    ...
    static RamScript build(RomProfile rom) {
        ...
        if (p != SIGNATURE_OFFSET)
            throw new IllegalStateException("Runtime v1 RC4 signature offset mismatch");
        ...
        if (p != PAYLOAD_OFFSET)
            throw new IllegalStateException("Runtime v1 RC4 payload offset mismatch");

        System.arraycopy(payload, 0, script, p, payload.length);
        p += payload.length;

        if (p != installerOffset)
            throw new IllegalStateException("Runtime v1 RC4 installer offset mismatch");
    ...
    */
    private static byte[] buildFinalWrapper(RomProfile rom) {
        if (rom.heldKeysRaw != 0x03003118L)
            throw new IllegalStateException("Runtime v1 RC4 assumes FR10 heldKeysRaw at 03003118");
    ...
    */
    ...
    /*
       RC4 visual Field Script payload.
    ...
    */
    private static byte[] buildFormatValidator() {
        return new byte[] {
            (byte)0x81, (byte)0x88,        // ldrh r1,[r0,#4*2 + 2] = [r0,#10]
            (byte)0xA7, 0x29,              // cmp r1,#0xA7
            0x6F, (byte)0xD1,              // bne 0300508E
    ...
    private static byte[] buildFieldScriptPayload() {
        ...
        if (p != 0x0F)
            throw new IllegalStateException("Runtime v1 RC4 text offset mismatch");
    ...
    private static void requireFr10(RomProfile rom) {
        if (rom != RomProfile.FIRE_RED_EN_10)
            throw new IllegalArgumentException("Native RamScript Runtime v1 RC4 is currently FR10-only");
    }
