/*
   Native RamScript Runtime v1 RC4aaa (FireRed English 1.0)
...
*/
final class NativeRamScriptRuntimeV1Rc4a {
    ...
    // gHostRfuGameData. RC4a uses 10 bytes for the hotkey safety gate.
    ...
    private NativeRamScriptRuntimeV1Rc4a() {}
    ...
    static RamScript build(RomProfile rom) {
        ...
        if (p != SIGNATURE_OFFSET)
            throw new IllegalStateException("Runtime v1 RC4aaa signature offset mismatch");
        ...
        if (p != PAYLOAD_OFFSET)
            throw new IllegalStateException("Runtime v1 RC4aaa payload offset mismatch");

        System.arraycopy(payload, 0, script, p, payload.length);
        p += payload.length;

        if (p != installerOffset)
            throw new IllegalStateException("Runtime v1 RC4aaa installer offset mismatch");
    ...
    */
    private static byte[] buildFinalWrapper(RomProfile rom) {
        if (rom.heldKeysRaw != 0x03003118L)
            throw new IllegalStateException("Runtime v1 RC4aaa assumes FR10 heldKeysRaw at 03003118");
    ...
    */
    private static byte[] buildSafetyGate() {
        /*
           ...
           RC4a uses the final two bytes as a local "reject tail" for the
           format validator:
             543E pop {r4,pc}
    ...
    */
    ...
    /*
       RC4a visual Field Script payload.
    ...
    */
    ...
    private static byte[] buildFieldScriptPayload() {
        ...
        if (p != 0x0F)
            throw new IllegalStateException("Runtime v1 RC4aaa text offset mismatch");
    ...
    private static void requireFr10(RomProfile rom) {
        if (rom != RomProfile.FIRE_RED_EN_10)
            throw new IllegalArgumentException("Native RamScript Runtime v1 RC4aaa is currently FR10-only");
    }
